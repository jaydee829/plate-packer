...
def test_absolute_coordinates_known_box():
    ...
    t4 = placement_transform((float(origin[0]), float(origin[1])), _identity_aff(), 0, 0, RES, PLATE)
    moved = box.copy()
...
def _shadow_setup(shift_mm=(0.0, 0.0)):
    ...
    mask, origin, _ = extract_footprint(box, RES)
    row = col = 100
    t4 = placement_transform((float(origin[0]), float(origin[1])), _identity_aff(), row, col, RES, PLATE)
    moved = box.copy()
    moved.apply_transform(t4)
...
def pack_command(
    ...
    if not no_verify:
        ...
        for path, occ in zip(plate_files, occs, strict=True):
            n = verify_plate(
                load_piece_mesh(path), occ, res, cfg.plate_mm, cfg.spacing_mm
            )
