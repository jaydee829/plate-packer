...
### 2026-08-07 - detector reach map lost a band to float32 + offset
- **Issue**: Support-aware footprints detected the cut depth wrong, landing too shallow. Real `*_supported.stl` tests showed 0% reduction; synthetic slab unit tests still passed (uncaught by the unit suite).
- **Root Cause**: The single-pass top-reach map computed `reach[px] = max_Z - z0 + 1.0` stored in a float32 canvas. Near ~1.5 mm (the reach value), float32 ULP (unit in last place) of ~1e-6 mm lost the sub-millimeter fraction; `area(d) = sum(reach > d)` then dropped raft-top pixels one `BAND_MM` (0.25 mm) early, cutting the body mask shallower than intended. The 0.25 mm error cascaded: the area-knee detector then found its knee at the wrong height, excluding too much of the raft.
- **Solution**: Store `max_Z - z0` (no +1.0 offset) in a **float64** canvas instead; `area(d) = reach > d` then preserves sub-mm precision and matches the float-compared body mask exactly. Regression: `test_detector_preserves_reach_precision` (reach map at different Z levels, area(d) matches expectations within floating-point epsilon). `footprint.py`, `tests/test_support_aware.py`.
- **Prevention**: Caught by the gated real-STL integration test (`example_stls` pytest marker, runs only when the junction to copyrighted STLs exists), not the unit tests. Lesson: synthetic unit tests (slab stacks) can hide floating-point precision issues; integration on real heterogeneous meshes is essential for numerics. Also: when you add a small constant to a float for "offset" reasons, validate it doesn't undermine precision for the actual computation.

### 2026-08-07 - angle_candidates emitted the MIRROR angle; contact_map np.rint crushed fractional edge_weight (PR #6 review round 2)
- **Issue**: Two quality bugs the GitHub auto-review caught after the first round. (a) `angle_candidates` did not axis-align *generic* hull edges — its per-edge angle had an inverted sign, so for e.g. an organic footprint the "shape-aware" angles were wrong; only axis-aligned/45° shapes (self-dual under θ↔180−θ) came out right. This is why the milestone's shape-aware angles under-delivered and `safety_grid=16` carried the win. (b) `contact_map`'s `np.rint(raw)` quantized fractional `edge_contact_weight` to whole numbers (e.g. 0.1 → `rint(0.5)` → 0), silently defeating the knob — a second instance of the same "test numeric knobs with fractions" lesson.
- **Root Cause**: (a) `base = (-degrees(atan2(dy,dx))) % 180` — the negation is wrong under `rotate_mask`'s convention `(x'=cos·x+sin·y, y'=-sin·x+cos·y)`, where zeroing the rotated edge's y-component gives `θ = +atan2(dy,dx)`. Verified: edge (2,1) → current 153.43° rotates it to (−1.34,−1.79) (not axis-aligned); fixed 26.57° → (2.24, 0.0). The sort-key fix in the prior round (`_analytic_aabb_area`) touched the ranking function but missed the sibling formula in candidate *generation*. (b) `np.rint` was chosen to collapse FFT noise for integer scores; it also erases fractional signal.
- **Solution**: (a) `base = degrees(atan2(dy,dx)) % 180` + a generic-tilted-edge regression test (rotate_mask bbox at the best candidate < 0.75× the 0° bbox) + a `cap < 1` guard. (b) `np.round(raw, 2)` (FFT noise measured <3e-4 on full-size plates → 2-decimal rounding is ~18×-safe) + a small-fraction regression test (0.1/0.2/0.3). `angles.py`, `packer.py`.
- **Prevention**: when TWO functions encode the same convention (candidate generation + its sort key), fix/verify BOTH — the first review pass fixed only the sort key. And (again) exercise numeric knobs with fractional, boundary values, not just integers/ordering.

### 2026-08-07 - Coarse-to-fine LOST a plate: the fine stage re-packed instead of realizing the coarse layout
- **Issue**: 30-piece pack — the coarse search fit all pieces on **4** plates (fitness 0.4962) but the fine stage returned **5** plates (0.3497), with a near-empty 5th plate holding 2 pieces. `fitness_initial == fitness_final` (the search added nothing at fine resolution). User: "went up a plate when we went fine???"
- **Root Cause**: the fine stage *discarded* the coarse placements and RE-PACKED the winning order with the fine contact-greedy packer. Greedy placement is not resolution-monotonic: the coarse 0.4 mm grid regularizes placement into 4 tidy plates, but at 0.1 mm the contact scorer has millions of positions and chases local shared-edge maxima, and with few shape-aware rotations (mean 4.7/piece; 5 circular bases got only 1) it spills the last pieces to a 5th plate — the same mechanism as the 8-rotation contact regression. coarse-legal⇒fine-legal guarantees the coarse layout is *realizable* at fine, but re-packing doesn't realize it.
- **Solution**: the fine stage now ALSO realizes the coarse layout — `_scale_placements` multiplies each coarse anchor by the resolution factor (collision-free & in-bounds by the block-max superset property plus a coarse-plate padding guard that marks partial edge cells occupied) — and keeps the better of {fine re-pack, realized-coarse} per ordering (`improve.py`). The realized layout can't lose plates to greedy myopia; density still improves with more rotations (e.g. `safety_grid`).
- **Prevention**: a multi-resolution search must be able to REALIZE the coarse solution, not merely rank orderings and re-solve — re-solving at a different resolution is a different problem and can regress. When an invariant (coarse-legal⇒fine-legal) says the coarse answer is realizable, realize it rather than recomputing.
