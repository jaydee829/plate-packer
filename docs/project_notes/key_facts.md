...
- **Detector constants** (in `footprint.py`): `BAND_MM = 0.25` (Z-level stepping for reach map), `MIN_REDUCTION = 0.05` (5% area drop required to cut), `FLAT_EPS = 0.01` (tolerance for flat cap detection), `DETECT_RES_MM = 0.2` (reach-map raster resolution), `DETECTOR_VERSION = 1` (bump this if any constant changes to trigger re-extract).
