...
## Support-Aware Footprints (v1.3, 2026-08-07, ADR-013)
- **Config knobs** (`[packing]`): `support_aware = false` (opt-in), `support_cut_cap_mm = 3.0` (knee search window AND hard ceiling on the raft-fusion zone, ADR-015). CLI `pack` gained `--support-aware` flag.
- **Detector constants** (in `footprint.py`): `BAND_MM = 0.25` (Z-level stepping for reach map), `MIN_REDUCTION = 0.05` (5% area drop required to cut), `FLAT_EPS = 0.01` (tolerance for flat cap detection), `DETECT_RES_MM = 0.2` (reach-map raster resolution), `DETECTOR_VERSION = 2` (bump this if any constant changes to trigger re-extract).
...
- **Raft-fusion packing (ADR-015)**: fused ⇔ body ≠ full (derived in `_pack_bounded` by mask comparison); fused bodies may nest over fused rafts, non-fused pieces stay strict full-shadow; plate boundary always uses the full. Caveat: cap 5→3 shipped without a DETECTOR_VERSION bump, so pre-existing caches keep cuts found under the 5 mm window — "all cuts ≤ 1.25 mm" is an empirical corpus property, not enforced; delete stale footprint caches when packing non-corpus STLs extracted before ADR-015.
- **Footprint-area-knee detector**: measures projected shadow area at each Z-level, finds `cut_z_mm` where area stops shrinking within the cap window, extracts `model_body` = shadow at Z ≥ cut_z_mm. No cut unless area drop ≥ `MIN_REDUCTION`. Measured **14–32% reduction** on real `*_supported.stl` corpus (Tome of Demons); gain is interior concavity (rafts hug model outline at ~0.0 mm outer flare).
- **Cache `SCHEMA_VERSION = 2`**: footprint JSON docs gained optional `model_body` entry (PNG-encoded mask) + `cut_z_mm` and `detector_version` metadata. Reads gracefully accept both v1 (no body field → falls back to full shadow) and v2 (reads body when present). One-STL-per-SHA-256 caching unchanged; content-address key unaffected.
- **Two-mask collision** (via `rotate_pair` shared canvas): inter-piece legality checks AND the piece against `model_body` mask (allows raft interior overlap — the feature's point). Plate-boundary checks AND the piece against the full shadow mask (raft outline must stay on-plate). Same-shape verification (body+full share a canvas); plate verification ORs the full mask at each anchor so legality is consistent. Implemented in `pack(..., boundary=)` and `improve(..., boundary_pieces=)` — `boundary=None` disables and runs byte-identical pre-feature code.
- **`example_stls` pytest marker**: test `test_support_aware_real_stls()` is decorated with `@pytest.mark.example_stls` and skips cleanly if the junction to copyrighted STLs is absent; deselected by default in CI/normal runs (pass `-m example_stls` to run). Gated real-STL tests are the only place numeric edge cases (float32 precision, area-knee false negatives) reliably surface — synthetic slab stacks won't catch them.
- **stl_curator coordination** (ADR-009 extension): the shared cache contract now specifies an optional `model_body` field. `stl_curator` should eventually emit `model_body` (SHA-256-keyed per STL, stored alongside intrinsic mask). `plate_packer` falls back gracefully to the full shadow until curator emits it — no breaking change to the contract, no re-extraction required on curator upgrade.
