...
- **Raft-fusion packing (ADR-015)**: fused ⇔ body ≠ full (derived in `_pack_bounded` by mask comparison); fused bodies may nest over fused rafts, non-fused pieces stay strict full-shadow; plate boundary always uses the full. Ceiling enforcement: `pack` treats a cached doc as stale when `cut_z_mm > support_cut_cap_mm` (as well as on version mismatch), so legacy caches from the 5 mm-cap era re-extract automatically — no manual cache deletion needed.
